...
module.exports = class Job {
  constructor (opts) {
    ...
    const randName = name + Math.random()
    Object.assign(this, {
      apiVersion: 'batch/v1',
      kind: 'Job',
      metadata: {
    ...
    if (opts.ports) {
      this.spec.containers[0].ports = opts.ports.map((port) => {
        return { containerPort: port }
      })
    }

    if (envs) {
      this.spec.containers[0].env = envs
    }

    if (opts.command) {
      this.spec.containers[0].command = opts.command
    }
  }
