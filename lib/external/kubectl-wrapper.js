...
module.exports = class KubectlWrapper {
   ...
   */
  deleteConfigs (name) {
    this.log.trace('deleteDeployment called')

    return Promise.resolve(this._generateDeleteConfigsCommand(name)).bind(this)
    .then(this._runCommand)
  }

  _generateDeleteConfigsCommand (name) {
    return `delete deployment,service,job -l name=${name} --output=name`
  }
  ...
  getServiceInfo (serviceName) {
    ...
    return Promise.resolve(this._generateGetServiceCommand(serviceName)).bind(this)
    .then(this._runCommand)
